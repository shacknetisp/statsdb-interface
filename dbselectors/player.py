...
class Selector(dbselectors.Selector):
    ...
    def fromrow(self, row):
        ...
        recentgames = 25
        try:
            recentgames = int(self.q['recentgames'][0])
        except:
            pass
        recentsums = 25
        try:
            recentsums = max(int(self.q['recentsums'][0]), 1)
        except:
            pass
        ret = {
            "recentgames": {},
        ...
        if self.flags['games']:
            ...
            if self.flags['recentgames']:
                ...
                gs.flags_none()
                for row in list(reversed(gamerows))[:recentgames]:
                    game = gs.fromrow(row)
                    ffarounds = []
        ...
        if self.flags['affinities']:
            ...
            for capture_row in self.db.execute(
                ...
                ORDER by ROWID DESC LIMIT %d)""" % (recentsums), (handle,)):
                    capture = {}
